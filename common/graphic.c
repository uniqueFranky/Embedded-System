...
/*---------------------------------------------------*/
	buf[(y << SCREEN_WIDTH_BIT) | x] = color;
/*---------------------------------------------------*/
	return;
}


// draw rect in range [x, x + w)  [y, y + h]
...
/*---------------------------------------------------*/
	...
	if(!width) {
		...
		if(height > width) {
			double ratio = 1.0 * height / (x2 - x1);
			if(y1 > y2) {
				swap(&y1, &y2);
			}
			for(int i = 0; i < height; i++) {
				buf[(x1 + (int)(i / ratio)) | ((y1 + i) << SCREEN_WIDTH_BIT)] = color;
			}

		} else {
			double ratio = 1.0 * width / (y2 - y1);
			if(x1 > x2) {
				swap(&x1, &x2);
			}
			for(int i = 0; i < width; i++) {
				buf[(x1 + i) | ((y1 + ((int)(i / ratio))) << SCREEN_WIDTH_BIT)] = color;
			}
		}
	}
/*---------------------------------------------------*/
